        ...
        $(document.body).insert(
            new Element('div', {id: 'popup_container'}).addClassName('dialog').insert(
                new Element('div').addClassName('content').insert(
                    ...
                    new Element('a', {id: 'close_link', 'onclick': 'Aulos.Popup.close(); return false;', 'onmouseover': "$('popup_container').setOpacity(0.7);", 'onmouseout': "$('popup_container').setOpacity(1.0);"}).addClassName('close_link').update('x')        
                ).insert(
    ...
    update: function(url, options) {
        ...
        new Ajax.Updater(
            ...
            url,
            {
                ...
                onComplete: function() { Aulos.Popup.rearrange(options.width) },
                onFailure: function(t) { Aulos.Utilities.Error.show(t.statusText); }
            }
        );
    },
